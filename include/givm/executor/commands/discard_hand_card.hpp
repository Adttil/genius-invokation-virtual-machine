...
namespace givm::detail
{
    ...
    inline execution_state finish_hand_card_discard_effect(
        ...
        execution_context& context, random_fn& random)
    {
        const auto event = get<0>(context.stack().top<hand_card_discarded, execution_position>());
        context.stack().pop<hand_card_discarded, execution_position>();
        context.enter_next();
        ...
        execution_context& context, random_fn& random)
    {
        ...
        if constexpr(Fixed)
        {
            card = context.instruction_data<1, discard_hand_card>(library).card;
            context.advance(instruction_extent<1, discard_hand_card>);
        }
        else
        {
            card = get<0>(context.stack().top<hand_card_discarded>()).card;
            context.stack().pop<hand_card_discarded>();
            context.enter_next();
        }
        ...
        if(not definition.can_handle<hand_card_discarded, hand_card_view>())
        {
            context.enter_next();
            prepare_broadcast(library, hand_card_discarded{ card }, table, context.stack(), context.position());
            return broadcast_hand_card_discard(library, table, context, random);
        }
        context.stack().push(hand_card_discarded{ card }, context.position());
        auto& event = get<0>(context.stack().top<hand_card_discarded, execution_position>());
        auto response = context.make_handle_context(table, random);
        const auto entry = definition.handle<hand_card_discarded>(self, event, response);
