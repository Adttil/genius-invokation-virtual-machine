...
| --- | --- |
| `player` | 装载牌组的玩家 |
| `deck` | 使用这张牌桌定义库中的 ID 的 [`linked_deck`](../../definition/linked_deck.md) |
...
该玩家的牌库和角色区必须尚未装载实体。卡牌按 `deck.cards` 的顺序装入，最后一张位于牌库顶；角色按 `deck.characters` 的顺序装入。该操作不执行角色初始化或开始对局。
