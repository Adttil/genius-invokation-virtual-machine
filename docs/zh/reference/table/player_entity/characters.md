[givm](../../../reference.md) / [牌桌](../../table.md) / [player_entity](../player_entity.md) / **characters**

# givm::player_entity::characters
...
产生 [`character_entity`](../character_entity.md) 的范围；实体具有与玩家访问对象相同的读写权限。
...
int main()
{
    ...
    std::println("移除前数量: {}", std::ranges::distance(player.characters()));
    entity.erase();
    std::println("移除后数量: {}", std::ranges::distance(player.characters()));
}
```
