...
切换出战角色的支付检查结果，区分骰子费用不匹配、持有骰子不足、充能类型不匹配与充能不足。
...
[`switch_payment_validate`](execution_view/action_selection/switch_payment_validate.md) 依次检查骰子费用匹配、持有数量、非零充能费用的类型匹配和充能数量，遇到第一个失败立即返回；全部通过时返回 `valid`。零充能费用忽略 `energy_tag`。检查不验证目标或其他行动条件。
