...
伤害扣除角色生命后的现场视图。可以读取本段伤害的结算结果、原始元素反应及其替代标签；此时本段的击倒处理、终局判定与默认反应实体生成尚未进行，而整组元素附着已在准备阶段推进。
...
若本次扣血使目标生命降至零，其 attachment 和充能在此现场仍保留；继续推进先广播 [`character_will_be_defeated`](../../definition/events/character_will_be_defeated.md)。响应程序结束后，生命非零表示复活成功；仍为零才判定终局。若对局结束，则保留这些数据并立即停止；若对局继续，才删除 attachment、清空充能与元素附着。

一组伤害可以多次到达本现场；此时只是当前一次扣血完成，不表示整组完成。当前段的默认反应实体尚未生成，牌桌可能已有前段生成且仍在场的实体。对局继续时，先处理当前段的击倒与未被替代的默认实体生成，再继续下一段伤害。
