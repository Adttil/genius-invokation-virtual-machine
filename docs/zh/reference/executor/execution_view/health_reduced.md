...
若本次扣血使目标生命降至零，其 attachment 和充能在此现场仍保留；继续推进先广播 [`character_will_be_defeated`](../../definition/events/character_will_be_defeated.md)。响应程序结束后，生命非零表示复活成功；仍为零才判定终局。若对局结束，则保留这些数据并立即停止；若对局继续，才删除 attachment、清空充能。
