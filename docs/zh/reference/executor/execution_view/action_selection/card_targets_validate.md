[givm](../../../../reference.md) / [执行](../../../executor.md) / [execution_view<action_selection>](../action_selection.md) / **card_targets_validate**
...
该牌对 [`card_target_validation`](../../../definition/queries/card_target_validation.md) 给出的结果：
...
没有对应查询时使用默认方法：目标数量为零时返回 `valid_complete`，否则返回 `invalid`，适用于无需目标的牌。

## 异常

牌定义的检查查询抛出的异常会传递给调用方。
