[givm](../../reference.md) / [执行](../executor.md) / **any_instruction_for**
...
适用于同一种 context 的同构指令值，容纳[核心给定集合](instructions.md)中的不同具体指令。它适合在运行时组装指令序列，例如根据对局配置决定需要哪些初始化操作。
...
保存指令的值，原指令对象在构造后可以销毁。可用指令及其参数要求见各[指令页面](instructions.md)。
