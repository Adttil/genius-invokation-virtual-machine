[givm](../../../reference.md) / [执行](../../executor.md) / [指令](../instructions.md) / **end_game**
...
以指定胜负结果结束对局的指令，可用于游戏流程或事件响应中的终局判定。
...
[`run`](../executor/run.md) 和 [`step`](../executor/step.md) 执行本指令后均返回 `execution_state::finished`，通过终局视图的 [`result()`](../execution_view/finished/result.md) 取得胜负。终局不能继续执行；本指令之后的程序步骤不再执行。

程序中先于本指令执行的步骤仍可提前结束对局，此时结果由先发生的终局决定。
