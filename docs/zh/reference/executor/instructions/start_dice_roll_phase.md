...
投骰前发出 [`dice_roll_preparation`](../events/dice_roll_preparation.md)。每方固定骰子总数不得超过 `count`，`count` 不得超过 [`selection_capacity`](../selection_capacity.md)。等待重投时按 `top<selector, stage_t>()` 取得输入槽，保留尾部状态；选择位按骰子种类顺序展开，同种骰子逐个计数。空选择表示放弃该方剩余重投。每次非空选择消耗一次重投机会。
