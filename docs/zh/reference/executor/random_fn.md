...
随机源无需采用特定生成算法，每次调用提供一个 `std::uint32_t` 值。一次执行推进中的指令和事件响应共享调用方传入的随机源，各次调用按实际执行顺序使用同一序列。随机值对应的游戏效果及消耗次序由各[指令](instructions.md)和定义响应约定。
