...
[`cost_of_switch`](../definition/events/cost_of_switch.md)、[`cost_of_card`](../definition/events/cost_of_card.md) 费用响应不得使用随机数；调用随机函数属于未定义行为。费用预览接口无需传入随机源。[定义查询](../definition/queries.md)不接收随机源。
