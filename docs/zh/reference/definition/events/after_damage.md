...
[`deal_damage`](../commands/deal_damage.md) 逐段完成伤害计算、扣血、击倒、元素附着及默认反应实体生成，整组结束后先完成已登记的超载切人及其通知，再按实际伤害顺序广播本事件。轮到某次伤害时，先完成它的元素反应后通知，再开始伤害后通知。击倒时已删除的 attachment 不参与本事件；若此前已判定终局，则不再广播本组尚未完成的通知。
