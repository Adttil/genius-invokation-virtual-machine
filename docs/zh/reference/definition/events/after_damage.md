...
[`deal_damage`](../commands/deal_damage.md) 先完成整组伤害的计算、扣血、击倒处理与元素附着，再按实际伤害顺序广播本事件。轮到某次伤害时，先完成它的元素反应后通知，再开始伤害后通知。击倒时已删除的 attachment 不参与本事件；若扣血后已判定终局，则不再广播本组尚未完成的通知。

响应函数在此时才被调用，读取的是当前牌桌；事件成员则保留对应伤害的结算结果。`reaction` 不根据此时的附着重新判断。前一个完成通知返回的程序可能继续修改牌桌，后面的通知会看到这些修改。
