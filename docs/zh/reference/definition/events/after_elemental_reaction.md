...
由伤害触发时，各段伤害先分别完成扣血、击倒、元素附着及默认反应实体生成；须等待整组结算结束，才在引发该反应的伤害后通知之前广播本事件。反应派生伤害也在这些完成通知之前结算；反应信息仍记录引发反应时的元素与附着。若此前已判定终局，则不再进行剩余完成通知。

独立 [`apply_element`](../commands/apply_element.md) 在相应反应处理中完成默认实体生成；其派生伤害也逐段完成扣血与附着，全部结算后再进行完成通知。没有发生反应时不广播本事件。
