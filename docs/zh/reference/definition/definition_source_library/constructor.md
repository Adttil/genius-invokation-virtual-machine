...
指定本场规则中草原核、激化领域、燃烧烈焰和冻结采用的定义，并登记这些源及同时传入的其他源。四个反应定义必传，可选用随库提供的[基础定义源](../../basic_definitions.md)，也可以提供自定义版本。
...
| `TFrozen` | `definition_category` 为 `attachment_view` 的定义源类型 |
...
| `frozen` | 冻结采用的源 |
| `other_sources...` | 一起登记的其他源，可用于满足前四者的依赖 |
...
四个默认反应定义由参数位置决定，不要求源使用特定名称或版本。它们及其依赖在部分选择时也始终保留；编译后可通过定义库的 [`dendro_core_id`](../../executor/definition_library/dendro_core_id.md)、[`catalyzing_field_id`](../../executor/definition_library/catalyzing_field_id.md)、[`burning_flame_id`](../../executor/definition_library/burning_flame_id.md)、[`frozen_id`](../../executor/definition_library/frozen_id.md) 取得对应 ID。
