...
两段流程只能使用[核心给定的指令](../../executor/instructions.md)，也可用 [`any_instruction_for`](../any_instruction_for.md) 保存。指令须与 `void` context 兼容。回合流程必须能够暂停或结束，避免空流程无限运行。定义源的编译操作抛出的异常继续向调用者传播。
