[givm](../../../reference.md) / [定义](../../definition.md) / [definition_compile_context](../definition_compile_context.md) / **add_program**
...
|  |  |
| --- | --- |
| `instructions` | 按顺序执行的[核心指令](../../executor/instructions.md)，也可用 [`any_instruction_for`](../any_instruction_for.md) 保存；每项须与 `TContext` 兼容并满足该容器的存储要求 |
...
返回入口只用于本次编译产生的定义库。效果执行完成后回到发起它的结算；本函数只登记效果，不立即执行。
