...
伤害结算命令。它为增伤、减伤和护盾等效果提供响应时机，并在扣除生命后处理伤害所携带的元素附着与胜负判定。
...
以 [`compile_mode::observed`](../../executor/compile_mode.md) 编译时，非零最终伤害扣除生命后先返回 `execution_state::health_reduced`；随后推进才处理元素附着及伤害后响应。相应[视图](../../executor/execution_view/health_reduced.md)的伤害值不以目标原有生命为上限。
