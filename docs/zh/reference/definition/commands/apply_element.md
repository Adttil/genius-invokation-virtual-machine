...
激化、燃烧、绽放分别在发生反应角色的对方请求生成定义库指定的激化领域、燃烧烈焰、草原核。归属由目标决定，与附着来源和当前行动玩家无关；为己方角色附着元素引起反应时，默认实体也生成在对方。已有同定义实体时按其重复生成响应处理，燃烧烈焰遵守召唤物容量限制。默认生成在对应反应的附着处理时完成；重复生成响应若提交程序，先完成它再继续后续伤害或完成通知。若反应被响应接管，则不执行默认生成。其他默认反应后果尚未全部实现。
