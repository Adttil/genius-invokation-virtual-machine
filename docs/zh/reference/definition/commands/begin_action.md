...
通过 [`calculate_card_cost`](../../executor/execution_view/action_selection/calculate_card_cost.md) 同步计算出牌费用，通过 [`check_card_payment`](../../executor/execution_view/action_selection/check_card_payment.md) 与 [`check_card_targets`](../../executor/execution_view/action_selection/check_card_targets.md) 分别检查支付及用牌条件。目标检查按 span 中的目标数量分步进行，允许检查空选择，告知当前选择是否有效、能否完成或继续；检查第二目标时可假设第一目标合法。两项检查相互独立，由调用方按需使用。费用初始化、费用响应与目标检查不得使用随机数，调用随机函数属于未定义行为。
