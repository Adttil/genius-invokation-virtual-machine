...
| `target` | [相对角色位置及作用范围](relative_damage_target.md)，默认对方出战角色 |
| `value` | 初始伤害值 |
...
处理每段描述时才解析来源和目标，找不到来源或存活目标时跳过该段。目标位置落在战败角色时，先循环向后寻找存活角色，再按范围选择该角色、其他角色或全部角色。范围目标按循环顺序逐个结算，仍属于同一伤害组；定位和展开规则见 [`relative_damage_target`](relative_damage_target.md)。
