...
- 默认构造 `modify_support_state{}` 使用动态模式，由 `invoke` 提交一个 [support_state_modification](../events/support_state_modification.md)。
...
例如，动态输入 `support_state_modification{ .support = target, .round_usages = -1 }` 会在本命令实际执行时扣除一次可用次数；另一个字段的增量默认是零。
