[givm](../../../reference.md) / [定义](../../definition.md) / [definition_library](../definition_library.md) / **has_all_tags**

# givm::definition_library::has_all_tags
...
满足条件时返回 `true`，否则返回 `false`。空标签范围返回 `true`。
...
int main()
{
    ...
    std::println("符合分类条件: {}", library.has_all_tags(card, tags));
}
```
