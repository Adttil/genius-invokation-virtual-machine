[开发备忘](../notes.md) / **随机预发、记录、回放与分支**

# 随机预发、记录、回放与分支

本页保留随机源放在每次调用边界的理由，以及预发、条带记录和模拟分叉的具体后果。代码片段展示记录器形态，不指定正式日志或持久化格式。调用者应遵守的数值映射、取得次序和预发规则分别写在对应指令的 reference 页中。

来源：提交 `b3d6c50` 中的 `docs/zh/old/random-input.md`。保留随机预发、输入次序与回放的设计约束。

随机性是每次同步调用的外部依赖，不是 executor 持有的生成器，也不是 table 中等待填写的状态。这个边界允许上层替换随机源、记录原始随机值、确定性回放和模拟分叉。
...
`calculate_switch_cost`、`calculate_card_cost` 及同步重新报价的行动提交重载不接收随机源，报价在调用中同步完成。费用 handler 保留统一签名，但不得调用随机函数，违反此前提属于未定义行为。角色初始状态与卡牌初始费用是空查询，结果在库编译期间保存；目标检查是非空查询，参数中不含随机源。反复预览或改变预览顺序不会消耗对局随机序列；下一次 `step` 只执行已确认结果，其中真正执行的后续效果仍可使用该次推进的随机源。详见[费用预览与提交](event_dispatch/payment_commit.md)。

随机函数的调用结果为 `std::uint32_t`；哪些响应应当使用随机数由对应事件的规则约定。

## 挂起与预发

推进返回 execution_state，表示本次到达的输入、观察现场或终局。到达现场之前仍可能取得随机数，不能把返回输入现场理解为本次没有执行其他工作。

需要让玩家提交顺序不影响结果时，指令可以在挂起前预发后续随机值并保存到自己的栈工作区。旧文把该工作区列入公开 stack ABI；当前把随机池及其布局作为内部状态，输入通过 execution_view 的参数式操作提交。例如开局换牌和双方重投会先准备各自的随机池，再逐个等待输入。
...
上述 `source_type` 是实际生成器的占位类型，`tape_random` 片段假定 `index < tape.size()`。工程中的回放器应自行定义耗尽条带时的处理方式；不能把越界读取当作核心的回放协议。选择后备生成器也属于上层分支策略，应在记录中能够区分原始条带和新产生的随机输入。

指令可以通过预发减少玩家提交先后顺序对随机分配的影响，但具体消耗量与转换方法由每条指令决定。仅保存生成器种子还依赖上层所选生成器的序列稳定性；原始 `std::uint32_t` 条带可记录实际消耗序列。规则版本之间若改变公开随机协议，属于影响回放兼容性的规则变更；单纯内部实现调整须保持公开的数值映射及消耗次序。

已预发的随机池是执行器快照的一部分，不能仅靠替换下一次传入的生成器改变它。避免把“换随机源”和“重新抽取已经确定的随机输入”视为同一操作。
