...
`calculate_cost` 同样以左值接收调用方的随机源，并通过统一的 handler 接口传给费用响应。费用 handler 按定义编写约定不得使用随机数，库不替换或拦截这一来源；执行行动前重新报价也透传当前 `step` 的随机源。正常费用响应遵守约定时，反复预览或改变预览顺序不会消耗对局随机序列，详见[费用预览与提交](event_dispatch/payment_commit.md)。
