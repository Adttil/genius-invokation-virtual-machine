...
下面结构体保留当时草图的 `card` 字段名；当前 [hand_card_status_id](../reference/table/hand_card_status_id.md) 与 [deck_card_status_id](../reference/table/deck_card_status_id.md) 均将该成员命名为 `card_id`。owner 加节点位置的设计不变。
...
**未采用的候选方案。**下列 `card_area` 和 `card_slots` 为设计草图，当前代码没有提供这套布局。
...
**待测试的候选方案。**当前并未实现下面的分段结构；AoS、alive_mask 和 N 的比较是原设计留下的实验方向。
...
以下保留作出取舍时的完整理由。它们说明本模型约束下为何未采用，不是通用数据结构优劣排名。
...
**原判断的适用限制：**下文“只有父子关系”是当时拒绝通用持久映射的前提，不足以证明当前所有内部引用都已处理。当前 `player_state::active_character` 就保存一个角色 ID，而 `player_data::clean_up()` 压缩角色后没有同步重映射它；此风险记录在[源码核对](reference_scope.md#独立于文档整理的源码问题)。仍应避免无用途的持久通用映射，但必须枚举并更新实际存在的引用，不能用旧前提跳过这一工作。
...
下面的默认卡牌/status 先后关系只描述 `detail::collect_all_broadcast_targets` 这项临时工具。事件并不统一规定全体广播；具体指令可以选择实体子集，也可以为同一事件安排不同响应顺序。广播机制另见[事件分派](event_dispatch.md)。
