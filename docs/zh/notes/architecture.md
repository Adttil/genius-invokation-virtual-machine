...
各段流程在编译结果中如何连接不属于公开接口。程序内部的跳转和返回由执行器收束，不单独交给外层驱动，因此对局运行期间，上层观察到的当前指令始终来自公开指令集。
...
指令和事件的字段均是公开接口：定义源需要直接提供指令，响应函数需要直接访问事件。其字段及使用约定由 reference 描述；完整帧与重入分支直接查阅源码。栈 ABI 的公开边界为何发生变化，统一见[设计演变与选择依据](history.md)，不在各主题重复维护旧约定。
