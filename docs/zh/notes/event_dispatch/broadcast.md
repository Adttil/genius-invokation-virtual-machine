...
默认广播在准备时采样响应者，保存事件与下一个响应者游标，并在帧尾保存 `response_return`，包含外层本方与返回位置。响应者的完整实体 ID 不作为后续程序隐含可读取的 Context；需要具体实体身份时仍由 handler 放入明确输入。响应返回非空入口时，调用方将 `table.state().self_player` 设为该响应者所属玩家，供固定效果命令的 `self`、`opponent` 使用。

响应程序末尾读取帧尾返回记录，恢复其中的外层本方并跳转，不弹出记录；同一次广播的所有响应程序共用此记录，广播结束时随广播帧一并清理。恢复时，执行位置指向广播推进入口，广播现场不保存 stage。handler 调用本身不切换本方，空入口不引入设置与恢复；内层响应程序按同样规则临时覆盖并恢复本方。
