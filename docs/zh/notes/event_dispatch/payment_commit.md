...
出牌沿用同一分工：`calculate_card_cost` 先读取定义库已保存的 `card_initial_cost` 结果，再依次调用 `cost_of_card` 响应并保存其 onpay。未提供源查询时，初始费用默认是零骰子的快速行动。`play_card(card_index, paid, targets)` 采用出牌候选索引对应的已报价结果，带定义库与牌桌的重载同步重报；目标 span 默认为空，至多复制前两个 ID，缺项补 `std::monostate`。最终选择保存两个固定目标 ID，不记录目标数量。
