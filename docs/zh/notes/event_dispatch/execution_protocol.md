...
这里记录维护指令和广播推进器时共同依赖的内部现场约束。执行上下文不属于公开接口；各指令自身的重入分支和临时布局直接查阅[指令源码](../../../../include/givm/executor/instructions)。公开观察与输入的边界见[执行观察与输入](../execution_observation.md)。

## 干净退出与 Context

干净退出约束指令如何管理自己建立的临时现场：一项结算可以经历多次重入并进入响应子程序，彻底离开该结算时必须恢复开始时的栈形状。进入终局时旧现场被逻辑废弃，栈顶只保留供结果视图读取的结果值，是明确例外。一次调用仍完整、同步地执行当前指令，不是在 C++ 函数中途挂起；这一区别及动态程序的历史表达方式见[一次完整执行与一次完整结算](../fixed_program.md#一次完整执行与一次完整结算)。
...
可继续执行的活动 frame 以内部 `stage_t` 结尾。stage 与对应的内部执行路径共同确定栈布局；终局结果不带 stage，不能按可重入现场解释。保留当前位置等待或进入子程序以前，指令必须先把栈调整为下一次重入可以解释的状态；不能依赖本次调用的局部变量在返回后继续存在。
