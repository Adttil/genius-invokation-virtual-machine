...
本篇记录为什么“同一张牌的数据”与“同一个区域实体”不能混为一谈，以及由此确定的弃牌、挑选和场上实体边界。搬运步骤和拒绝理由完整保留，用于修改区域模型时检查是否破坏已有假设。当前公开访问方式以[实体的身份与访问](../reference/table/entity_access.md)为入口。
...
本文描述实体与转移语义，以及弃牌、挑选的设计边界。内部存储布局的取舍见 [Table 内部数据结构设计记录](table_storage.md)。
...
对应只读访问对象为 [hand_card_status_view](../reference/table/hand_card_status_view.md) 和 [deck_card_status_view](../reference/table/deck_card_status_view.md)；声明订阅与安装响应的区别见 [subscribed_events](../reference/definition/subscribed_events.md) 和[定义源协议](../reference/definition/source_protocol.md#事件响应)。
...
定义类别与可用 view 的映射见 [views_of_definition](../reference/definition/views_of_definition.md)。以下保留一份 definition 配两组 handler 表的实现动机。
