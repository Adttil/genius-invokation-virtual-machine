...
| 字段 | 类型 | 含义 |
| --- | --- | --- |
| `state` | `character_state` | 将写回角色实体的初始状态。 |
...
这个流程是普通 handler 机制的一种特例：每次调用只涉及当前角色，事件对象不跨指令，因此可以放在局部变量而不是 executor stack 上。handler 使用五参数固定接口，但当前约定只修改 `state` 并返回 null；调用指令会忽略返回入口。以后可为这个约定增加更严格的静态检查。
