...
| `multiplier_numerator` | `std::uint16_t` | 最终统一应用的倍率分子。 |
| `multiplier_denominator` | `std::uint16_t` | 最终统一应用的倍率分母。 |
| `type` | `damage_type` | 伤害类型。 |
| `flags` | `damage_flags` | 伤害规则标记。 |

## 执行

本命令入口不要求特定栈顶布局，也不直接读取输入栈。执行期间会临时压入 `damage_calculation`、`damage_effect` 和 `after_damage` 广播 frame；彻底完成时会恢复入口栈形状。
...
4. 生成并推进 [`damage_effect`](../events/damage_effect.md) 广播。
5. 根据最终 `damage_effect` 扣除目标生命。
